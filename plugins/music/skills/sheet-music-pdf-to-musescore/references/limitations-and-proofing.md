...
| Most chord symbols missing from the export | Expected: OCR mangles a digit, Audiveris cannot parse the chord name, and demotes it to plain text. Don't hand-correct them — run `scripts/place_chords_from_pdf.py`, which reads all of them from the PDF text layer and places them by the score's own system layout. |
| Two chords in one bar print stacked vertically | They were placed at the same time position. A chord must be interleaved between notes to sit later in the bar — MuseScore ignores `<offset>` on harmony. `place_chords_from_pdf.py` does this; the beat may be approximate but is always a real onset. |
| A chord appears twice, once above each staff | Harmony was cleared from only the top part. It must be removed from every part before inserting; chord symbols belong to the top part alone. |
